...
# Load your saved pipeline (which includes preprocessing and the model)
try:
...
with rate_col3:
    ...
    # We still need to pass 'Gender' even if we don't ask for it,
    # as the model was trained on it. We'll use a common default.
    gender = 'Female' # You can also make this an st.selectbox if you want
...
if st.button('Predict Satisfaction', use_container_width=True, type="primary"):
    ...
    # The keys MUST match the column names from your original 'X_train' DataFrame
    input_data = {
        'Gender': gender,
        ...
        # Note: 'Arrival Delay in Minutes' was dropped, so the pipeline won't expect it.
    }
    ...
    # Make prediction
    # The model pipeline handles all preprocessing (scaling, encoding)
    try:
